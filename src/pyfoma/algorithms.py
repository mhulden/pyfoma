    ...
    for s in fst.states:
        ...
        for lbl in s.transitions.keys():
            if any(l in lbl for l in map):
                newlabel = tuple(
                    map[lbl[i]] if lbl[i] in map else lbl[i] for i in range(len(lbl))
                )
...
def epsilon_removed(fst: "FST") -> "FST":
    ...
    if all(len(ec) == 0 for ec in eclosures.values()):  # bail, no epsilon transitions
        return fst.__copy__()
    newfst, mapping = fst.copy_filtered(
        labelfilter=lambda lbl: any(len(sublabel) != 0 for sublabel in lbl)
    )
        ...
        for target, cost in ec.items():
            ...
            for label, t in target.all_transitions():
                if all(len(sublabel) == 0 for sublabel in label):  # is epsilon: skip
                    continue
                mapping[state].add_transition(
                    mapping[t.targetstate], label, cost + t.weight
                )
...
def words_cheapest(fst: "FST"):
    ...
    while Q:
        ...
        if s is None:
            ...
            for label, t in s.all_transitions():
                heapq.heappush(
                    Q, (cost + t.weight, next(cntr), t.targetstate, seq + [label])
                )
...
def determinized_as_dfa(fst: "FST") -> "FST":
    ...
    return determinized.copy_mod(
        modlabel=lambda l, _: l[:-1], modweight=lambda l, _: l[-1]
    )
...
def determinized(fst: "FST", staterep=lambda s, w: (s, w), oplus=min) -> "FST":
    ...
    while Q:
        ...
        for label, tset in collectlabels.items():
            ...
            newQ = frozenset(
                staterep(t.targetstate, t.weight + residuals[s] - wprime)
                for s, t in tset
            )
            ...
            if any(t.targetstate in fst.finalstates for _, t in tset):
                ...
                newstate.finalweight = oplus(
                    t.targetstate.finalweight + t.weight + residuals[s] - wprime
                    for s, t in tset
...
def merging_equivalent_states(fst: "FST", equivalenceclasses: set) -> "FST":
    ...
    newfst.finalstates = {
        statemap[s] for s in fst.finalstates if s in representerstates
    }
    for s in fst.finalstates:
        if s in representerstates:
    ...
    for s in fst1.finalstates:
        for lbl2, t2 in ocopy.initialstate.all_transitions():
            q1q2[s].add_transition(
                q1q2[t2.targetstate], lbl2, t2.weight + s.finalweight
            )
...
def compose(fst1: "FST", fst2: "FST") -> "FST":
    ...
    while Q:
            ...
            if mode == 0 or matchsym != "":  # A=x:y B=y:z, or x:0 0:y (only in mode 0)
                for outtrans in A.transitionsout.get(matchsym, ()):
                    for intrans in B.transitionsin.get(matchsym, ()):
                        ...
                        currentstate.add_transition(
                            S[(target1, target2, 0)],
                            newlabel,
                            outtrans[1].weight + intrans[1].weight,
                        )
        for outtrans in A.transitionsout.get("", ()):  # B waits
            ...
            currentstate.add_transition(
                S[(target1, target2, 1)], newlabel, outtrans[1].weight
            )
        for intrans in B.transitionsin.get("", ()):  # A waits
            ...
            currentstate.add_transition(
                S[(target1, target2, 2)], newlabel, intrans[1].weight
            )
    return newfst
...
def rewritten(fst: "FST", *contexts, **flags) -> "FST":
    ...
    if len(contexts) > 0:
        ...
        defs["rule"] = center.context_restrict(*lrpairs, rewrite=True).compose(
            defs["base"]
        )
    else:
        defs["rule"] = defs["base"]
    ...
    defs["worsen"] = (
        functools.reduce(lambda x, y: x.union(y), worseners)
        .determinize_unweighted()
        .minimize()
    )
    defs["rewr"] = FST.re("$^output($^input($rule) @ $worsen)", defs)
    final = FST.re("(.* - $rewr) @ $rule", defs)
    newfst = (
        final.map_labels({s: "" for s in ["@<@", "@>@", "#"]})
        .epsilon_remove()
        .determinize_as_dfa()
        .minimize()
    )
...
def projected(fst: "FST", dim=0) -> "FST":
    ...
    if (
        "." not in newalphabet
    ):  # Preserve . semantics if it occurs on the tape we extract
    ...
    for s, lbl, t in fst.all_transitions(fst.states):
        ...
        if t.targetstate in fst.finalstates:
            newfst.initialstate.add_transition(
                mapping[s], lbl, t.weight + t.targetstate.finalweight
            )
    return newfst
...
def union(fst1: "FST", fst2: "FST") -> "FST":
    ...
    for lbl, t in itertools.chain(
        fst1.initialstate.all_transitions(), fst2.initialstate.all_transitions()
    ):
        newfst.initialstate.add_transition(mapping[t.targetstate], lbl, t.weight)
    # Also add all transitions from old FSMs to new FSM
    for s, lbl, t in itertools.chain(
        fst1.all_transitions(fst1.states), fst2.all_transitions(fst2.states)
    ):
    ...
    if fst1.initialstate in fst1.finalstates or fst2.initialstate in fst2.finalstates:
        newfst.finalstates.add(newfst.initialstate)
        newfst.initialstate.finalweight = min(
            fst1.initialstate.finalweight, fst2.initialstate.finalweight
        )
...
def intersection(fst1: "FST", fst2: "FST") -> "FST":
    ...
    return product(
        fst1, fst2, finalf=all, oplus=operator.add, pathfollow=lambda x, y: x & y
    )
...
def difference(fst1: "FST", fst2: "FST") -> "FST":
    ...
    return product(
        fst1,
        fst2,
        finalf=lambda x: x[0] and not x[1],
        oplus=lambda x, y: x,
        pathfollow=lambda x, y: x,
    )
...
@_harmonize_alphabet
def product(
    fst1: "FST", fst2: "FST", finalf=any, oplus=min, pathfollow=lambda x, y: x | y
) -> "FST":
    ...
    while Q:
        ...
        for lbl in pathfollow(t1s.transitions.keys(), t2s.transitions.keys()):
            for outtr in t1s.transitions.get(
                lbl, (Transition(dead1, lbl, float("inf")),)
            ):
                for intr in t2s.transitions.get(
                    lbl, (Transition(dead2, lbl, float("inf")),)
                ):
                    ...
                    currentstate.add_transition(
                        S[(outtr.targetstate, intr.targetstate)],
                        lbl,
                        oplus(outtr.weight, intr.weight),
                    )
    return newfst
