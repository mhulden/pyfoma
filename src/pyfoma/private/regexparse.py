...
class RegexParse:
    ...
    builtins = {'reverse': lambda x: alg.reversed(x),
                ...
                'input': lambda x: alg.projected(x, dim = 0),
                'output': lambda x: alg.projected(x, dim = -1)}
        ...
        for op, value, line_num, column in self.parsed:
            if op == 'FUNC':
                ...
                if len(rng) == 1:  # e.g. {3}
                    ...
                    lang = alg.optional(lang)
                    _append(stack, functools.reduce(lambda x, y: alg.concatenate(x, y), [lang]*int(rng[1])))
                elif rng[1] == '': # e.g. {3,}
                    _append(stack, functools.reduce(lambda x, y: alg.concatenate(x, y), alg.concatenate([lang]*int(rng[0])), alg.kleene_closure(lang)))
                else:              # e.g. {1,4}
                    if int(rng[0] > rng[1]):
                    ...
                    lang2 = functools.reduce(lambda x, y: alg.concatenate(x, y), [alg.optional(lang)]*(int(rng[1])-int(rng[0])))
