...
class FST:
                ...
                for label in s.transitions.keys():
                    for transition in s.transitions[label]:
                        st += '{}\t{}\t{}\t{}\n'.format(statenums[id(s)],
                                                        statenums[id(transition.targetstate)], '\t'.join(
                                                            label),
                                                        transition.weight)
        for s in self.states:
    ...
    def cleanup_sigma(self):
        ...
        seen = {sym for _, lbl, _ in self.all_transitions(
            self.states) for sym in lbl}
        if '.' not in seen:
            self.alphabet &= seen
        return self
    ...
    def view(self, raw=False, show_weights=False, show_alphabet=True) -> 'graphviz.Digraph':
        ...
        g = graphviz.Digraph('FST', graph_attr={
                             "label": sigma, "rankdir": "LR"})
        ...
        for s in self.finalstates:
            ...
            if s == self.initialstate:
                g.node(
                    str(statenums[id(s)]) + _float_format(s.finalweight), style='filled, bold')
        ...
        for s in self.states:
            if s not in self.finalstates:
                ...
                if s == self.initialstate:
                    g.node(str(statenums[id(s)]),
                           shape='circle', style='filled, bold')
            grouped_targets = defaultdict(set)  # {states}
            for label, t in s.all_transitions():
                grouped_targets[t.targetstate] |= {
                    (t.targetstate, label, t.weight)}
            for target, tlabelset in grouped_targets.items():
                if raw == True:
                    labellist = sorted((str(l) + '/' + str(w)
                                       for t, l, w in tlabelset))
                else:
                    labellist = sorted(
                        (':'.join(_str_fmt(label)) + _float_format(w) for _, label, w in tlabelset))
                printlabel = ', '.join(labellist)
                if s in self.finalstates:
                    sourcelabel = str(
                        statenums[id(s)]) + _float_format(s.finalweight)
                else:
                    sourcelabel = str(statenums[id(s)])
                if target in self.finalstates:
                    targetlabel = str(
                        statenums[id(target)]) + _float_format(target.finalweight)
                else:
                    targetlabel = str(statenums[id(target)])
                g.edge(sourcelabel, targetlabel,
                       label=graphviz.nohtml(printlabel))
        return g
    ...
    def copy_mod(self, modlabel=lambda l, w: l, modweight=lambda l, w: w):
        ...
        for s, lbl, t in self.all_transitions(q1q2.keys()):
            q1q2[s].add_transition(q1q2[t.targetstate], modlabel(
                lbl, t.weight), modweight(lbl, t.weight))
    ...
    def apply(self: 'FST', word, inverse=False, weights=False, tokenize_outputs=False, obey_flags=True, print_flags=False):
        ...
        IN, OUT = [-1, 0] if inverse else [0, -
                                           1]  # Tuple positions for input, output
        cntr = itertools.count()
        w = self.tokenize_against_alphabet(word)
        Q, output = [], []
        # (cost, -pos, output, state)
        heapq.heappush(Q, (0.0, 0, next(cntr), [], self.initialstate))
        ...
        while Q:
            ...
            if state == None and -negpos == len(w) and (not obey_flags or flag_filter(output)):
                ...
                yield_output = ''.join(
                    output) if not tokenize_outputs else output
                if weights == False:
                    yield yield_output
                else:
                ...
                if state in self.finalstates:
                    heapq.heappush(Q, (cost + state.finalweight,
                                   negpos, next(cntr), output, None))
                for lbl, t in state.all_transitions():
                    if lbl[IN] == '' or FlagOp.is_flag(lbl[IN]):
                        heapq.heappush(
                            Q, (cost + t.weight, negpos, next(cntr), output + [lbl[OUT]], t.targetstate))
                    elif -negpos < len(w):
                        nextsym = w[-negpos] if w[-negpos] in self.alphabet else '.'
                        appendedsym = w[-negpos] if (nextsym ==
                                                     '.' and lbl[OUT] == '.') else lbl[OUT]
                        if nextsym == lbl[IN]:
                            heapq.heappush(Q, (
    ...
    def tokenize_against_alphabet(self: 'FST', word) -> list:
        ...
        while start < len(word):
            # Default is length 1 token unless we find a longer one
            t = word[start]
...
class State:
    def __init__(self, finalweight=None, name=None):
        __slots__ = ['transitions', '_transitionsin',
                     '_transitionsout', 'finalweight', 'name']
        # Index both the first and last elements lazily (e.g. compose needs it)
        # (l_1,...,l_n):{transition1, transition2, ...}
        self.transitions = dict()
        # l_1:(label, transition1), (label, transition2), ... }
        self._transitionsin = None
        # l_n:(label, transition1), (label, transition2, ...)}
        self._transitionsout = None
    ...
    def rename_label(self, original, new):
        ...
        for t in self.transitions[original]:
            t.label = new
        self.transitions[new] = self.transitions.get(
            new, set()) | self.transitions[original]
    ...
    def remove_transitions_to_targets(self, targets):
        ...
        for label, transitions in self.transitions.items():
            newt[label] = {
                t for t in transitions if t.targetstate not in targets}
            if len(newt[label]) == 0:
                newt.pop(label)
    ...
    def add_transition(self, other, label, weight):
        ...
        self.transitions[label] = self.transitions.get(label, set()) | {
            newtrans}
    ...
    def all_epsilon_targets_cheapest(self) -> dict:
        ...
        for lbl, tr in self.transitions.items():
            if all(len(sublabel) == 0 for sublabel in lbl):  # funky epsilon-check
                for s in tr:
                    targets[s.targetstate] = min(
                        targets[s.targetstate], s.weight)
        return targets
